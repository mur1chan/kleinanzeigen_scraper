...
def calculate_and_display_statistics():
    ...
    for item in data:
        product_table.insert('', 'end', values=(item['title'], item['price'], item['location'], item['upload'], item['link']))


def open_link(event):
    item = product_table.focus()
    item_dict = product_table.item(item)
...
product_entry.grid(row=0, column=1, padx=(0, 20), pady=(20, 10))

min_price_label = tk.Label(tab1, text="Min Preis:")
min_price_label.grid(row=1, column=0, padx=(20, 10), pady=(0, 10))
min_price_entry = tk.Entry(tab1)
min_price_entry.grid(row=1, column=1, padx=(0, 20), pady=(0, 10))

max_price_label = tk.Label(tab1, text="Max Preis:")
max_price_label.grid(row=2, column=0, padx=(20, 10), pady=(0, 10))
max_price_entry = tk.Entry(tab1)
max_price_entry.grid(row=2, column=1, padx=(0, 20), pady=(0, 10))

start_button = tk.Button(tab1, text="Start", command=start_scraper_thread)
start_button.grid(row=3, column=0, columnspan=2, pady=(0, 20))
...
calculate_button = tk.Button(tab2, text="Statistik berechnen", command=calculate_and_display_statistics)
...
median_price_label.pack()

product_table = ttk.Treeview(tab2, columns=('title', 'price', 'location', 'upload', 'link'), show='headings')
product_table.heading('title', text='Titel')
product_table.heading('price', text='Preis')
product_table.heading('location', text='Standort')
product_table.heading('upload', text='Hochgeladen')
product_table.heading('link', text='Link')
product_table.pack(pady=(20, 0), fill='x')
product_table.bind('<Double-1>', open_link)
